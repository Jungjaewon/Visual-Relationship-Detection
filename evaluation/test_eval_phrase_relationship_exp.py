...
def parse_args():
	...
	parser.add_argument('--det_json_file', dest='det_json_file', help='object detection result',default='', type=str)
	parser.add_argument('--gt_json_file', dest='gt_json_file', help='gt test json file', default='', type=str)
	parser.add_argument('--zgt_json_file', dest='zgt_json_file', help='gt zeroshot json file', default='', type=str)
	parser.add_argument('--gpu', dest='gpu_id', help='GPU id to use',default=0, type=int)
	parser.add_argument('--def', dest='prototxt',help='prototxt file defining the network',default='/home/woodcook486/sigmoid_spatial_word/prototxt/spatial_word_test.prototxt', type=str)
	parser.add_argument('--net', dest='caffemodel',help='model to test',default='/home/woodcook486/sigmoid_spatial_word/_iter_70000.caffemodel', type=str)
...
def test_net(net, jsonfile, prd_map, out_dir, caffemodel, thresh, test_mode, smode):
	...
	image_root = '/home/user01/py-faster-rcnn/data/vrd/Images/'
	test_result_k1 = {}
	test_result_k10 = {}
	test_result_k70 = {}
	...
	with open(jsonfile,'r') as fp:
		obj_det_result = json.load(fp)
	with open(prd_map,'r') as fp:
		prd_mapping = cp.load(fp)
	iteration = caffemodel.split('/')[-1]
	#f_k1 = open(out_dir + iteration + '_k1_relph.txt','w')
	#f_k10 = open(out_dir + iteration + '_k10_reph.txt','w')
	#f_k70 = open(out_dir + iteration + '_k70_reph.txt','w')
	...
	cnt = 0
	len_Rel = len(obj_det_result)
	for image in obj_det_result:
		if (cnt + 1) % 100 == 0:
			print 'relph test : ', cnt + 1, '/', len_Rel
		filename = image['filename']
		im = cv2.imread(image_root + filename).astype(np.float32, copy=False)
		ih = im.shape[0]
		iw = im.shape[1]
		object_pair_list = image['object_pairs']
		...
		for object_pair in object_pair_list:
			spatials = []
			ims = []
			pair_vectors = []
			factors = []

			sbj_name = object_pair[0]['name']
			obj_name = object_pair[1]['name']
			sbj_bbox = object_pair[0]['bbox']
			obj_bbox = object_pair[1]['bbox']
			sbj_conf = object_pair[0]['confidence']
			obj_conf = object_pair[1]['confidence']

			# test_mode 0 only visual , 1 visual + spatial , 2 visual + wordvector , 3 visual + spatial + word vector
			factor = float(sbj_conf) * float(obj_conf)
			...
			choosed_k10 = np.argsort(batch_result)[::-1][:10]
			...
			for i in range(len(choosed_k10)):
				if check_prd_k10.has_key(prd_mapping[choosed_k10[i]]):
					check_prd_k10[prd_mapping[choosed_k10[i]]] += 1
				else:
					check_prd_k10[prd_mapping[choosed_k10[i]]] = 1
				spo_list_k10.append([sbj_name, prd_mapping[choosed_k10[i]],obj_name])
				cnf_list_k10.append([sbj_conf,batch_result[choosed_k10[i]],obj_conf])
				sobox_list_k10.append([sbj_bbox,obj_bbox])
				#pstr_k10 += (prd_mapping[choosed_k10[i]] + ' ' + "{0:.3f}".format(batch_result[choosed_k10[i]]) + ' ')

			for i in range(len(choosed_k70)):
				spo_list_k70.append([sbj_name, prd_mapping[choosed_k70[i]],obj_name])
				cnf_list_k70.append([sbj_conf,batch_result[choosed_k70[i]],obj_conf])
				sobox_list_k70.append([sbj_bbox,obj_bbox])
				#pstr_k70 += (prd_mapping[choosed_k70[i]] + ' ' + "{0:.3f}".format(batch_result[choosed_k70[i]]) + ' ')

			#f_k1.write('{} / {} , relationships : {} {} {}\n'.format(str(cnt + 1), str(len_Rel), sbj_name,  obj_name, pstr_k1))
			#f_k10.write('{} / {} , relationships : {} {} {}\n'.format(str(cnt + 1), str(len_Rel), sbj_name, obj_name, pstr_k10))
			#f_k70.write('{} / {} , relationships : {} {} {}\n'.format(str(cnt + 1), str(len_Rel), sbj_name, obj_name, pstr_k70))

		test_result_k1[filename] = [spo_list_k1,cnf_list_k1,sobox_list_k1]
		test_result_k10[filename] = [spo_list_k10,cnf_list_k10,sobox_list_k10]
		test_result_k70[filename] = [spo_list_k70,cnf_list_k70,sobox_list_k70]
		cnt += 1

	#f_k1.write(json.dumps(check_prd_k1,indent=2))
	#f_k10.write(json.dumps(check_prd_k10,indent=2))
	#f_k1.close()
	#f_k10.close()
	#f_k70.close()
	...
	return test_result_k1,test_result_k10,test_result_k70

def evalutaion_phrase(json_file, phrase_test_result, maximum_det, ov_thresh, k, caffemodel, out_dir, Zeroshot = False):

	assert k != ''
	with open(json_file,'r') as fp:
		Rel = json.load(fp)

	testResult = phrase_test_result
	tp = []
	fp = []
	score = []
	total_gts = 0
    ...
    #assert len(Rel) == len(testResult)

	for rel in Rel:

		if (cnt + 1) % 100 == 0:
			pass
			#print 'eval phrase : ',cnt + 1, '/', len(Rel)
		filename = rel['filename']
		filen = filename.split('.')[0]
		filename = '.'.join([filen,'jpg'])
		gtRelationships = rel['relationships']
		num_gts = len(gtRelationships)
		total_gts += num_gts
		gt_detected = np.zeros(num_gts)

		#if filename not in testResult:
		#	continue
		assert filename in testResult
		assert len(testResult[filename]) == 3
		spo_list = testResult[filename][0]
		conf_list = testResult[filename][1]
		unibox_list = testResult[filename][2]
		assert len(spo_list) == len(conf_list) == len(unibox_list)

		for u in range(len(unibox_list)):
			unibox_list[u] = getUnionBB(unibox_list[u][0],unibox_list[u][1])
		conf_list = np.array(conf_list)
		spo_list = np.array(spo_list)
		unibox_list = np.array(unibox_list)

		if isinstance(conf_list, np.ndarray) and len(conf_list) > 0:
			det_score = np.log(conf_list[:,1] + 1e-8 )
			...
			for j in xrange(num_dets):
				...
				for s in xrange(num_gts):
					...
					if gt_detected[s] == 0 and top_spo[j][0] == phrase[0] and top_spo[j][1] == phrase[1] and top_spo[j][2] == phrase[2]:
						ov = computeIoU(top_box[j],getUnionBB(gt_sbjbox,gt_objbox))
	...
	if Zeroshot is False:
		print 'phrase Recall@',str(maximum_det), ' k= ',str(k), ' : ', top_recall, ' ', str(iteration)
	elif Zeroshot is True:
		print 'phrase Zeroshot Recall@',str(maximum_det), ' k= ',str(k), ' : ', top_recall, ' ', str(iteration)

	if Zeroshot is False:
		result_txt = 'phrase_Recall@' + str(maximum_det) + '_k'+str(k)+'.txt'
	elif Zeroshot is True:
		result_txt = 'Zeroshot_phrase_Recall@' + str(maximum_det) + '_k'+str(k)+'.txt'

	with open(out_dir + result_txt,'a') as fp:
		if Zeroshot is False:
			fp.write('phrase_dectection Recall@' + str(maximum_det) +  ' k= ' + str(k) + ' : ' + str(top_recall) + ' ' + str(iteration) + '\n')
		elif Zeroshot is True:
			fp.write('phrase_dectection Zeroshot Recall@' + str(maximum_det) +  ' k= ' + str(k) + ' : ' + str(top_recall) + ' ' + str(iteration) + '\n')



def evalutaion_relationship(json_file, relationship_test_result, maximum_det, ov_thresh, k, caffemodel, out_dir,Zeroshot = False):

	assert k != ''
	with open(json_file,'r') as fp:
		Rel = json.load(fp)
	
	testResult = relationship_test_result
	tp = []
	fp = []
	score = []
	total_gts = 0
	...
	for rel in Rel:
		if (cnt + 1) % 100 == 0:
			pass
			#print 'eval relationship : ',cnt + 1, '/', len(Rel)
		filename = rel['filename']
		filen = filename.split('.')[0]
		filename = '.'.join([filen,'jpg'])
		gtRelationships = rel['relationships']
		...
		spo_list = testResult[filename][0]
		conf_list = testResult[filename][1]
		sobox_list = testResult[filename][2]
		...
		if isinstance(conf_list, np.ndarray) and len(conf_list) > 0:
			...
			for j in xrange(num_dets):
				...
				for s in xrange(num_gts):
					...
					gt_objbox = gtRelationships[s]['object']
					if gt_detected[s] == 0 and top_spo[j][0] == phrase[0] and top_spo[j][1] == phrase[1] and top_spo[j][2] == phrase[2]:
						ov = min(computeIoU(top_box[j][0],gt_sbjbox),computeIoU(top_box[j][1],gt_objbox))
						if ov >= ov_thresh and ov > ov_max:
							ov_max = ov
							arg_max = s
				if arg_max != -1:
	...
	if Zeroshot is False:
		print 'relationship Recall@',str(maximum_det), ' k= ',str(k), ' : ', top_recall, ' ', str(iteration)
	elif Zeroshot is True:
		print 'relationship Zeroshot Recall@',str(maximum_det), ' k= ',str(k), ' : ', top_recall, ' ', str(iteration)

	if Zeroshot is False:
		result_txt = 'relationship_Recall@' + str(maximum_det) + '_k'+str(k)+'.txt'
	elif Zeroshot is True:
		result_txt = 'Zeroshot_relationship_Recall@' + str(maximum_det) + '_k'+str(k)+'.txt'

	with open(out_dir + result_txt,'a') as fp:
		if Zeroshot is False:
			fp.write('relationship_dectection Recall@' + str(maximum_det) +  ' k= ' + str(k) + ' : ' + str(top_recall) + ' ' + str(iteration) + '\n')
		elif Zeroshot is True:
			fp.write('relationship_dectection Zeroshot Recall@' + str(maximum_det) +  ' k= ' + str(k) + ' : ' + str(top_recall) + ' ' + str(iteration) + '\n')
...
if __name__ == '__main__':
	...
	test_result_k1,test_result_k10,test_result_k70 = test_net(net, args.det_json_file,args.prd_map,args.out_dir, args.caffemodel,args.ov_thresh, args.mode, args.smode)

	evalutaion_phrase(args.gt_json_file, deepcopy(test_result_k1), args.maximum_det, args.ov_thresh, '1', args.caffemodel, args.out_dir,   Zeroshot = False)
	evalutaion_phrase(args.gt_json_file, deepcopy(test_result_k10), args.maximum_det, args.ov_thresh, '10', args.caffemodel, args.out_dir, Zeroshot = False)
	evalutaion_phrase(args.gt_json_file, deepcopy(test_result_k70), args.maximum_det, args.ov_thresh, '70', args.caffemodel, args.out_dir, Zeroshot = False)
	evalutaion_phrase(args.zgt_json_file, deepcopy(test_result_k1), args.maximum_det, args.ov_thresh, '1', args.caffemodel, args.out_dir,   Zeroshot = True)
	evalutaion_phrase(args.zgt_json_file, deepcopy(test_result_k10), args.maximum_det, args.ov_thresh, '10', args.caffemodel, args.out_dir, Zeroshot = True)
	evalutaion_phrase(args.zgt_json_file, deepcopy(test_result_k70), args.maximum_det, args.ov_thresh, '70', args.caffemodel, args.out_dir, Zeroshot = True)

	evalutaion_relationship(args.gt_json_file, deepcopy(test_result_k1), args.maximum_det,  args.ov_thresh, '1', args.caffemodel, args.out_dir,  Zeroshot = False)
	evalutaion_relationship(args.gt_json_file, deepcopy(test_result_k10), args.maximum_det, args.ov_thresh, '10', args.caffemodel, args.out_dir, Zeroshot = False)
	evalutaion_relationship(args.gt_json_file, deepcopy(test_result_k70), args.maximum_det, args.ov_thresh, '70', args.caffemodel, args.out_dir, Zeroshot = False)
	evalutaion_relationship(args.zgt_json_file, deepcopy(test_result_k1), args.maximum_det,  args.ov_thresh, '1', args.caffemodel, args.out_dir,  Zeroshot = True)
	evalutaion_relationship(args.zgt_json_file, deepcopy(test_result_k10), args.maximum_det, args.ov_thresh, '10', args.caffemodel, args.out_dir, Zeroshot = True)
	evalutaion_relationship(args.zgt_json_file, deepcopy(test_result_k70), args.maximum_det, args.ov_thresh, '70', args.caffemodel, args.out_dir, Zeroshot = True)
